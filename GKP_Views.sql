...
GROUP_CONCAT(Unique_Diagnosis_Number,Name_Of_Diagnosis, ' '), Exercise_Program,
...
select group_concat(Second_Name, First_Name, Middle_Name, ' '),
group_concat('Номер:', Sick_List_Number, 'Дата открытия:', Sick_List_Opening_Date,
...
group by Sick_List_Number, Second_Name, First_Name, Middle_Name;
...
--Предстоящие записи
CREATE VIEW `new_visit` AS select Ticket_Number, Date_Of_Reception, Time_Of_Reception from ticket where Date_Of_Reception > sysdate();
...
from Sick_List
where (select datediff(Sick_List_Closing_Date, Sick_List_Opening_Date)) > (select avg(datediff(Sick_List_Closing_Date, Sick_List_Opening_Date)) from Sick_list);
